...
const ConnectButton = ({ userId, profileId, onSuccess }: ConnectButtonProps) => {
  ...
  if (done) {
    return (
      <button disabled className="w-full h-9 rounded-[4px] border border-border text-muted-foreground/70 text-[12px] font-semibold">
        Pending
      </button>
    );
  }
  ...
  return (
    <button
      ...
      className="w-full h-9 rounded-[4px] border border-border text-foreground/90 hover:bg-secondary text-[12px] font-semibold flex items-center justify-center transition-colors"
    >
...
export function SuggestedConnections({ suggestions, currentUserId, onRemove }: SuggestedConnectionsProps) {
  ...
  return (
    <div className="bg-card text-card-foreground border border-border rounded-[6px] p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 text-foreground font-bold text-[15px]">
          <Users className="h-5 w-5 text-primary" />
          Suggested Connections
        </div>
        <Link to="/explore" className="text-[13px] font-semibold text-primary hover:underline flex items-center">
          See all
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-none snap-x">
        {suggestions.map((s) => (
          <div key={s.id} className="flex flex-col items-center min-w-[160px] snap-start border border-border/50 bg-card text-card-foreground p-5 text-center rounded-[6px]">
